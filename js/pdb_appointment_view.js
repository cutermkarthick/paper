...
//$("#appointment4dateloader").load(base_url+"patient_ctrl/getappts4date/"+date.format());

	///document.getElementById('light').style.display='block';document.getElementById('fade').style.display='block';

                   
    },
            eventLimit: true, // allow "more" link when too many events:: changed on 28th march 2015
            events: window.location.origin + '/paperless/patient_ctrl/cal_events'
        });
...
function getdateval(valk)
{
	...
	if (waittime.indexOf(',') > -1)
	{
		...
		{
		alert("Your appointment is under waiting list. Is that ok?") ;
				   ...
				   $select.find('option').remove();
					...
					$.each(calender_slots, function (key, value) 
					{
						...
						document.getElementById('waittime').value =waitarr ;
							...
							$('<option enabled="enabled" style="background-color:#DFDFDF">').val(val).text(key).appendTo($select);
...
if(document.forms[0].appt_duration.value == '')
	...
	if (waittime.indexOf(',') > -1)
		...
		if(waittime.indexOf(apptime) > - 1)
			{
			var confirm1 = confirm("Your appointment is under waiting list. Is that ok?") ;
			if(!confirm1)
			{
				return false ;
			...
			document.getElementById('status').value = 'Waitlist' ;
			break;
			}
		}
	else if(apptime == waittime)
