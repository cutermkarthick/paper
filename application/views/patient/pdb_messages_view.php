...
// Filename: pdb_messages_view.php             =
// Copyright of Fluent Technologies            =
// Displays messages for a patient             =
//==============================================
?>
...
<?php
$attributes = array('class' => 'form-horizontal_4', 'name'=>'form-horizontal_4','id' => 'form-horizontal_4', 'style' => 'padding-top:0px;');
echo form_open('patient_ctrl/messages', $attributes);
?>
...
$('#date'+id).css("font-weight", "normal");
		...
		$.get("<?php echo base_url();?>patient_ctrl/edit_mail/"+inb_rec+"/"+type, function (msg) 
		{
...
{
	$.get("<?php echo base_url();?>patient_ctrl/replay_mail/"+inb_rec+"/"+type, function (msg) 
	{
	   $('#maildetails').html(msg);
	});
}
...
{ 
	...
	{
		$.get("<?php echo base_url();?>patient_ctrl/edit_mail/"+sent_rec+"/sent", function (msg) 
		{
		   $('#maildetails').html(msg);
		});
	}
...
else
{
  if(inb_rec == '')
	...
	{
		$.get("<?php echo base_url();?>patient_ctrl/edit_mail/"+inb_rec+"/inbox", function (msg) 
		{
		   $('#maildetails').html(msg);
		});
	}
...
$('#myTab li').click(function()  
{
	...
	$("#sends").css({"display": "none"});
           ...
           else if(param == 'sent1')
	   {
		...
		{
			$.get("<?php echo base_url();?>patient_ctrl/edit_mail/"+sent_rec+"/sent",          
function (msg) 
			{
			   $('#maildetails').html(msg);
			});
...
var sent_index=document.getElementById('sent_index').value;
	if (r == true) 
	{
		$.get("<?php echo base_url();?>patient_ctrl/delete_email/"+sent_index, function (msg)
		{ 
		})
	}
	else
...
?>
&nbsp;&nbsp;
<span class="input-append date" id="dp4"  data-date-format="yyyy-mm-dd">
<?
$data1 = array(
	'type' => "text",
	'name' => "from_date",
...
<span class="input-append date" id="dp5"  data-date-format="yyyy-mm-dd" style="margin-left:-15px">
<?
$data2 = array(
	'type' => "text",
	'name' => "to_date",
...
echo form_open('patient_ctrl/delete_email4inb', $attributes);
?>
<thead>
<tr>
<th colspan="6">
...
if($q->read_flag =='0')
{
	...
	{?>
		<tr id="<?= $inb_id?>"  class="clickableRow" href="javascript:edit_mail('<?= $q->recnum ?>','inbox')" >
		<th> <input name="<?= $chk ?>" id="<?= $chk ?>"  type="checkbox" value="" onclick="geteach_inbval('<?= $i ?>')"></th>
	...
	{?>
	<tr  id="<?= $inb_id?>"  class='clickableRow' href="javascript:edit_mail('<?= $q->recnum ?>','inbox')" >
		<td> <input name="<?= $chk ?>" id="<?= $chk ?>"  type="checkbox" value="" onclick="geteach_inbval('<?= $i ?>')"></td>
		<td> &nbsp;</td>
		<?
	 if($q->attachment !='')
	{?>
...
echo form_open_multipart('patient_ctrl/insert_email', $attributes);
?>
<div class="row-fluid patient_history">
...
<label class="control-label" for="inputEmail">Doctors:</label>
<div class="controls">
<?php
$attributes = 'id="patient_name" class="input-xlarge"  onchange="getpatient_name(this)" ';
$patientnames  = $this->patient_model->getdocdetails4patient();
...
<div class="controls">
<input type="file" name="userfile" id='userfile' onchange="this.style.width = '200px ';" > 
</div>
</div>
...
<input type='hidden' name='p_name' id='p_name' value="<?=$this->session->userdata('patient_name') ?>">
<div class="controls">
...
echo form_open('patient_ctrl/messages/sent', $attributes);
?>
<div style="margin-top:0px;" class="control-group">
<div class="controls" style='margin-left:0px'>
<?php
...
?>
&nbsp;&nbsp;
<span class="input-append date" id="dp6"   data-date-format="yyyy-mm-dd">
<?
$data = array(
	'type' => "text",
	'name' => "fromdate_sent",
...
<span class="input-append date" id="dp7"  data-date-format="yyyy-mm-dd" style="margin-left:-20px">
<?
$data = array(
	'type' => "text",
	'name' => "todate_sent",
...
echo form_open('patient_ctrl/delete_email', $attributes);
?>
<thead>
<tr>
<th colspan="6">
